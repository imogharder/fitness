...
function showProgress(entries) {
  ...
  // Need at least 2 entries, sorted by date
  ...
  fields.forEach((field) => {
    ...
    if (latestVal === null || prevVal === null || latestVal === undefined || prevVal === undefined) {
      return;
    }
    ...
    const display = `${sign}${diff.toFixed(1)} ${field.unit}`;

    const div = document.createElement('div');
    div.className = 'result-item';
    div.innerHTML = `
      <span class="label">${field.label}</span>
      <span class="value ${cssClass}">${display}</span>
    `;
